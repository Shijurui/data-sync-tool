...
for path in MONITORED_DIRS:
    for file in os.listdir(path):
        ...
        if file_exists:
            ...
            db_last_modified_time = cur.execute(
                f"SELECT time_last_change FROM files WHERE file_path = '{file_path}'"
            ).fetchone()[0]
        ...
        if file_exists == False:
            cur.execute(
                f"INSERT INTO files VALUES ('{file_path}', '{file_hash}', '{last_modified_time}', 'NULL', 'Y')"
            )
        else:
            # File has been modified, save to database
            cur.execute(
                f"UPDATE files SET file_hash = '{file_hash}', time_last_change = '{last_modified_time}', to_sync = 'Y' WHERE file_path = '{file_path}'"
            )
        con.commit()
